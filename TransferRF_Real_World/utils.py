...
def average_value(stored_value):
    ...
    try:
        if isinstance(average_score, (int, float)) and not (math.isnan(average_score) or math.isinf(average_score)):
            average_score = Decimal(average_score).quantize(Decimal('0.00000'), rounding=ROUND_HALF_UP)
        else:
...
def median_value(stored_value):
    ...
    median_score = Decimal(median_score).quantize(Decimal('0.00000'), rounding=ROUND_HALF_UP)
    return median_score
...
def IQR(stored_value):
    ...
    iqr = q3 - q1
    iqr = Decimal(iqr).quantize(Decimal('0.00000'), rounding=ROUND_HALF_UP)
    return iqr

def standard_deviation(x):
    std = np.std(x)
    std = Decimal(std).quantize(Decimal('0.00000'), rounding=ROUND_HALF_UP)
    return std
    
def round_number(x):
    round_x = Decimal(x).quantize(Decimal('0.00000'), rounding=ROUND_HALF_UP)
    return round_x
...
def loss_plot(adam_history, problem_selection, save_path, bsf_value):
    '''
    ...
    complete_name = os.path.join(save_path, f"{problem_selection}_Loss"'.png')
...
def plot_sigma_values(sigma_values, save_path, run_number):
    plt.figure(figsize=(10, 5))
    plt.plot(sigma_values, label='Sigma over Iterations')
    plt.title(f'Sigma Value Progression in Run {run_number}')
    plt.xlabel('Iteration')
    plt.ylabel('Sigma Value')
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{save_path}/sigma_progress_run_{run_number}.png")
    plt.close()  # Close the figure to free memory

def plot_and_save_histogram(data, save_path, k, file_identifier):
    plt.figure(figsize=(10, 6))
    ...
    plt.grid(True)
    file_path = os.path.join(save_path, f'distributions_{k}_{file_identifier}.pdf')
